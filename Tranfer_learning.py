...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25, save_path='model/resnet18_test/'):
    since = time.time()
    ...
    with torch.no_grad():
        for i, (inputs, labels) in enumerate(dataloaders['val']):
            ...
            for j in range(inputs.size()[0]):
                ...
                ax.axis('off')
                ax.set_title('predicted: {}\nlabel: {}'.format(class_names[preds[j]].split('-')[1], class_names[labels[j]].split('-')[1]))
...
if __name__ == '__main__':
    ...
    model_ft = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler,
                            num_epochs=epoch, save_path='model/resnet50_false')

    # model_ft.load_state_dict(torch.load('./model/resnet50_model.pth',
    #                                    map_location='cuda:3'))
